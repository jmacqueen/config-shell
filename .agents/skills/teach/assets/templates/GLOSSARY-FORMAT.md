...
- **Group under subheadings** when natural clusters emerge, such as `## Anatomy` or `## Programming`.
- **Flag ambiguities explicitly.** If a term is used loosely in the wider field, note the resolution.
- **Revise as understanding deepens.** Update definitions in place rather than leaving stale entries.
